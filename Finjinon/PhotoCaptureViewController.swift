...
public class PhotoCaptureViewController: UIViewController, PhotoCollectionViewLayoutDelegate {
    ...
    override public func viewDidLoad() {
        ...
        flashButton.titleLabel?.font = UIFont.preferredFont(forTextStyle: UIFontTextStyleFootnote)
        ...
        pickerButton.titleLabel?.font = UIFont.preferredFont(forTextStyle: UIFontTextStyleFootnote)
    ...
    public func dequeuedReusableCellForClass<T : PhotoCollectionViewCell>(_ clazz: T.Type, indexPath: IndexPath, config: ((T) -> Void)) -> T {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: clazz.cellIdentifier(), for: indexPath) as! T
        config(cell)
        return cell
    }

    public func reloadPreviewItemsAtIndexes(_ indexes: [Int]) {
    ...
    public func selectedPreviewIndexPath() -> IndexPath? {
    ...
    public func cellForpreviewAtIndexPath<T : PhotoCollectionViewCell>(_ indexPath: IndexPath) -> T? {
    ...
    public func previewRectForIndexPath(_ indexPath: IndexPath) -> CGRect {
    ...
    public func createAssetFromImageData(_ data: Data, completion: (Asset) -> Void) {
        storage.createAssetFromImageData(data, completion: completion)
    }

    public func createAssetFromImage(_ image: UIImage, completion: (Asset) -> Void) {
        storage.createAssetFromImage(image, completion: completion)
    }

    public func createAssetFromImageURL(_ imageURL: URL, dimensions: CGSize, completion: (Asset) -> Void) {
    ...
    public func deleteAssetAtIndex(_ idx: Int, handler: () -> Void) {
    ...
    private func didAddAsset(_ asset: Asset) {
        DispatchQueue.main.async {
            self.collectionView.performBatchUpdates({
    ...
    public func photoCollectionViewLayout(_ layout: UICollectionViewLayout, canMoveItemAtIndexPath indexPath: IndexPath) -> Bool {
...
extension PhotoCaptureViewController: UICollectionViewDataSource, PhotoCollectionViewCellDelegate {
    ...
    func collectionViewCellDidTapDelete(_ cell: PhotoCollectionViewCell) {
        if let indexPath = collectionView.indexPath(for: cell) {
            self.deleteAssetAtIndex((indexPath as NSIndexPath).item, handler: {
